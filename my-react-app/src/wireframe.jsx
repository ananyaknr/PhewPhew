  ...
  return (
    <Box 
      style={{ 
        ...
        position: "relative",
        boxSizing: "border-box", // Prevents padding from breaking width/height
        overflow: "hidden" // Ensures the main wrapper never scrolls
      }}
    >
      <StatusBar />
      ...
      {/* Illustration */}
      <div style={{ width: "100%", height: "18vh", minHeight: 100, maxHeight: 180, padding: "0 20px", marginBottom: 12, flexShrink: 0, boxSizing: "border-box" }}>
      ...
      <Box 
        style={{ 
          ...
          overflowY: "auto", 
          padding: "0 20px 16px",
          minHeight: 0, // CRITICAL FIX: Allows flexbox to shrink and actually use the scrollbar
      ...
      >
        ...
        {step === 0 && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(80px, 1fr))", gap: 8 }}>
              {[["◎", "Scan Ingredients"], ["◈", "AI Analysis"], ["◆", "Earn Rewards"]].map(([icon, label]) => (
                <div key={label} style={{ padding: "10px 4px", borderRadius: 12, background: C.surface, border: `1px solid ${C.border}`, textAlign: "center" }}>
                  <Text size={18}>{icon}</Text>
                  <Text size={9} color={C.sub} style={{ marginTop: 4, lineHeight: 1.3 }}>{label}</Text>
                </div>
              ))}
        ...
        {step === 1 && (
          <>
            ...
            {/* min(100%, 220px) ensures it fits narrow screens without breaking */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))", gap: 10 }}>
              {[
                { val: "Female", desc: "Higher estrogen — tends toward dryness", icon: "♀" },
                { val: "Male", desc: "Higher androgens — oilier, thicker skin", icon: "♂" },
                { val: "Non-binary", desc: "We'll use general skin data", icon: "◈" },
              ].map(opt => (
                <div
                  key={opt.val}
                  onClick={() => setGender(opt.val)}
                ...
                >
                  ...
                  {gender === opt.val && <div style={{ width: 18, height: 18, borderRadius: "50%", background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Text size={9} color="white">✓</Text></div>}
                </div>
              ))}
            </div>
          </>
        ...
        {step === 2 && (
          <>
            <Text size={22} weight={800} style={{ fontFamily: "Syne, sans-serif", lineHeight: 1.2, marginBottom: 6 }}>What do you think<br />your skin type is?</Text>
            <div style={{ padding: "10px 12px", borderRadius: 12, background: C.accentMid + "44", border: `1px solid ${C.accentMid}`, marginBottom: 14, boxSizing: "border-box" }}>
              <Text size={11} color="#1A5A78" weight={600}>💡 Just your best guess is perfect.</Text>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))", gap: 10 }}>
              {[
                { val: "Oily", desc: "Shiny by midday, enlarged pores", emoji: "💧" },
                { val: "Dry", desc: "Feels tight, flaky patches", emoji: "🌵" },
                { val: "Combination", desc: "Oily T-zone, normal cheeks", emoji: "⚖️" },
                { val: "Sensitive", desc: "Reacts easily, redness", emoji: "🌸" },
                { val: "Normal", desc: "Balanced, rarely breaks out", emoji: "✨" },
              ].map(opt => (
                <div
                  key={opt.val}
                  onClick={() => setSkinType(opt.val)}
                ...
                >
                  ...
                  {skinType === opt.val && <div style={{ width: 18, height: 18, borderRadius: "50%", background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Text size={9} color="white">✓</Text></div>}
                </div>
              ))}
            </div>
          </>
        ...
        {step === 3 && (
          <>
            <Text size={22} weight={800} style={{ fontFamily: "Syne, sans-serif", lineHeight: 1.2, marginBottom: 8 }}>What are your<br />skin goals?</Text>
            <Text size={13} color={C.sub} style={{ marginBottom: 16, lineHeight: 1.5 }}>Pick everything that matters to you.</Text>
            <ChipSelector
              options={["Reduce acne", "Anti-aging", "Even tone", "Hydration", "Brightening", "Pores", "Barrier repair", "Reduce redness", "Firming"]}
              selected={goals}
              onToggle={toggleGoal}
            />
          </>
        ...
        {step === 4 && (
          <>
            <Text size={22} weight={800} style={{ fontFamily: "Syne, sans-serif", lineHeight: 1.2, marginBottom: 8 }}>Where do you<br />spend your days?</Text>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))", gap: 10 }}>
              {[
                { val: "Office / Indoors", desc: "AC environments cause dehydration", icon: "🏢" },
                { val: "Mixed", desc: "Variable conditions", icon: "🔄" },
                { val: "Outdoor", desc: "High UV, heat, sweat", icon: "🌤" },
                { val: "Work from Home", desc: "Controlled climate but sedentary", icon: "🏠" },
              ].map(opt => (
                <div
                  key={opt.val}
                  onClick={() => setWorkEnv(opt.val)}
                ...
                >
                  ...
                  {workEnv === opt.val && <div style={{ width: 18, height: 18, borderRadius: "50%", background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Text size={9} color="white">✓</Text></div>}
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))", gap: 10 }}>
              {[
                ...
                { val: "Low Pollution", desc: "Urban but cleaner air", icon: "🌆", tag: null },
                { val: "Suburban", desc: "Moderate exposure", icon: "🏘", tag: null },
                { val: "Rural / Nature", desc: "Fresh air, high UV", icon: "🌿", tag: null },
                { val: "Coastal", desc: "Salt air + high humidity", icon: "🌊", tag: null },
              ].map(opt => (
                <div
                  key={opt.val}
                  onClick={() => setSurrounding(opt.val)}
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <Text size={13} weight={700} color={surrounding === opt.val ? "#0D4A6A" : C.text}>{opt.val}</Text>
                      {opt.tag && <div style={{ padding: "2px 6px", borderRadius: 99, background: "#FDE8E8", border: "1px solid #F5AAAA", fontSize: 8, fontWeight: 700, color: C.danger }}>{opt.tag}</div>}
                    </div>
                    <Text size={11} color={C.sub}>{opt.desc}</Text>
                  </div>
                  {surrounding === opt.val && <div style={{ width: 18, height: 18, borderRadius: "50%", background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Text size={9} color="white">✓</Text></div>}
                </div>
              ))}
            </div>
          </>
        ...
        {step === 6 && (
          <>
            <Text size={22} weight={800} style={{ fontFamily: "Syne, sans-serif", lineHeight: 1.2, marginBottom: 8 }}>Any allergies<br />or sensitivities?</Text>
            <Text size={11} weight={700} color={C.sub} style={{ letterSpacing: "0.08em", marginBottom: 10 }}>KNOWN ALLERGENS</Text>
            <ChipSelector
              options={["Fragrance", "Paraben", "Alcohol", "Sulfate", "Retinol", "AHA / BHA", "Essential Oils", "Lanolin"]}
              selected={allergies}
              onToggle={toggleAllergy}
            />
      ...
      {/* Bottom CTA */}
      <div style={{ padding: "12px 20px 24px", flexShrink: 0, boxSizing: "border-box" }}>
        <div
          onClick={() => {
            if (!canProceed() && step !== 0 && step !== 6) return;
            if (step === TOTAL_STEPS - 1) onNav("home");

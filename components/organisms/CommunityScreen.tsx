  ...
  const posts = [
    {
      ...
      product: { name: "COSRX Advanced Snail 96 Mucin", price: "฿490", match: true, matchReason: "Suits combination skin — lightweight, non-comedogenic." },
      week1: { hydration: 60, clarity: 62, texture: 64 }, week4: { hydration: 78, clarity: 80, texture: 82 },
    },
    {
      user: "DermNerd", score: 91, review: "Reminder: never mix retinol + vitamin C in same AM routine. Use Vit C in AM and retinol only PM.", upvotes: 289,
      beforeScore: 75, afterScore: 91, skinType: "Normal/Dry",
      product: { name: "The Ordinary Retinol 0.5%", price: "฿350", match: true, matchReason: "Start slow with 0.5% — ideal for beginners." },
        ...
        {posts.map((p, i) => (
          <Box key={i} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "16px", marginBottom: 14 }}>
            <Box style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 18, background: `linear-gradient(135deg, ${C.accentMid}, ${C.mint})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span>✨</span>
              </div>
              <div style={{ flex: 1 }}>
            ...
            <div style={{ background: C.bg, borderRadius: 12, padding: "10px 12px", marginBottom: 10, border: `1px solid ${C.border}` }}>
              <Text size={10} weight={700} color={C.sub} style={{ marginBottom: 8 }}>Skin Analysis · Before → After</Text>
              {[["Hydration", p.week1.hydration, p.week4.hydration], ["Clarity", p.week1.clarity, p.week4.clarity]].map(([label, before, after]) => (
                <div key={label as string} style={{ marginBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                    <Text size={9} color={C.sub}>{label}</Text>
                    <Text size={9} color={C.accent} weight={600}>{before} → {after}</Text>
                  </div>
